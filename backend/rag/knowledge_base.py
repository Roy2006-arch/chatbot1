...
class KnowledgeBase:
    ...
    def _cached_encode(text_tuple: tuple) -> np.ndarray:
        embedder = ModelRegistry.get_embedder()
        return embedder.encode(list(text_tuple), batch_size=1, show_progress_bar=False)
    ...
    def _embed(self, texts: list[str]) -> np.ndarray:
        if len(texts) == 1:
            vecs = self._cached_encode(tuple(texts))
        else:
    ...
    def _maybe_upgrade_index(self) -> None:
        if self._index.ntotal >= _IVF_THRESHOLD and isinstance(self._index, faiss.IndexFlatIP):
            logger.info("[KnowledgeBase] Upgrading index to IVF...")
        ...
        if index_path.exists() and metadata_path.exists():
            try:
                ...
                if self._tokenized_corpus:
                    self._bm25 = BM25Okapi(self._tokenized_corpus)
                logger.info("[KnowledgeBase] Loaded %d vectors from disk", self._index.ntotal)
    ...
    def clear(self) -> None:
        with self._lock:
            ...
            self._bm25 = None
            self._save()
        logger.info("[KnowledgeBase] Cleared all data.")
